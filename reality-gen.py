...
1.  **Locate Target Entity and Trace Path:** Parse the semantic path provided by the user. Recursively search through the `primary_constituents` arrays (at the top level and within any existing `manifestation` blocks) to find the entity that matches the last part of the path, ensuring the full path is consistent. **During this search, you MUST meticulously construct the exact, verified semantic path (an array of entity names) leading to the located target entity. This path MUST solely reflect the actual hierarchy traversed within the provided world JSON, not any external or imagined context.**
    ...
    *   **CRITICAL PATH ACCURACY:** The generated `path` in the output MUST be an **absolute, verifiable trace** of the actual `name` values of the entities from the world's root down to the located entity. **DO NOT invent, alter, or hallucinate path elements.** Path elements MUST strictly correspond to the `name` field of entities found in the JSON hierarchy. If a parent entity (including the top-level `world` object) does not possess a `name` field, that parent entity's name MUST NOT be included in the `path` array. The `path` should therefore begin directly with the `name` of the first *named* descendant entity. **Crucially, any entity directly listed within the top-level `world` object's `primary_constituents` array is considered a direct child of the unnamed `world` root. Its `name` will be the first element in the `path` if it's part of the target entity's hierarchy, and you MUST NOT infer or assume any *other* named entity (like a sibling constituent) as its parent unless explicitly defined by nested `manifestation` blocks.** For example, if a "Component X" is found within "Assembly Y", the path MUST be `["Assembly Y", "Component X"]`.
