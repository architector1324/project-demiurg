...
SRGE предлагает мощный набор возможностей, призванных оживить сложные реальности:
  ...
  * **Глубокая онтологическая структура:** Каждый сгенерированный мир представлен в виде **структурированного JSON-вывода**, тщательно определяющего его существование по пяти основным измерениям. Это не просто текст; это чертёж живого мира:
      * `world_essence`: Фундаментальная природа и определяющая концепция реальности.
  ...
  * **Реальность как наблюдение:** Миры, созданные SRGE, по своей сути бесконечно сложны. Однако, подобно принципам квантовой механики, их детали "материализуются" только при наблюдении. LLM выступает в роли **"проявителя" присущей сложности**, раскрывая последующие уровни детализации мира, которые уже изначально заложены в его фундаментальной семантической структуре, а не генерируются произвольно. Это гарантирует, что даже при глубоком исследовании мир остаётся абсолютно внутренне непротиворечивым и верным своим изначальным законам.
...
Для запуска SRGE локально вам потребуется установленный и настроенный **Ollama**, а также модель `qwen3:4b`.
...
| -- | -- | :--- |
| `--new`     | Генерирует совершенно новую, высокоуровневую реальность из короткого промпта.                                                                                         | **Готово** |
| `--explore` | Исследует существующую реальность на основе её уже сгенерированного JSON, позволяя задавать вопросы или извлекать факты. **Эта команда не изменяет состояние мира.** | *В процессе* |
| `--deep`    | Погружается в конкретный компонент или подсистему существующего мира и рекурсивно семантически детализирует её. **Мир детализируется автоматически, раскрывая свою присущую сложность.** | *В процессе* |
| `--live`    | Экспериментально: Симулирует динамическую эволюцию и события внутри сгенерированного мира на основе его внутренних сил и правил.                                        | *Запланировано* |



## Примеры использования

Для генерации нового мира (используя команду `--new`):

#### Онтология фундаментальной физики:

```bash
python reality-gen.py --new "Две частицы в вакууме"
```
...
```json
{
  "world_essence": "Вселенная, определяемая взаимодействием двух фундаментальных частиц в пустом пространстве, управляемая законами квантовой механики и релятивистской физики.",
...
```bash
python reality-gen.py --new "Волшебный мир с рыцарями, магией и злодеями"
```

```json
{
  "world_essence": "Мир, где магия пронизывает ткань бытия, управляемый взаимодействием света и тени. Рыцари служат хранителями баланса, в то время как тёмные силы стремятся нарушить гармонию мира.",
  "primary_constituents": [
    {
      "name": "Этерион",
