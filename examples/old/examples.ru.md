...
```bash
python srge.py create "Горящая свеча в темной комнате (научно точно)" --model qwen3:4b
```
...
```bash
python srge.py navigate 'Пламя' --model qwen3 --input initial_candle_world.json
```
...
**1.1. Создание Мира (Начальный `create`):**
```bash
python srge.py create "Капля воды в вакууме (научно точно)" --model qwen3:4b
```

**1.2. Навигация (Последующий `navigate`):**
*(Использует JSON, сгенерированный в шаге 1.1, в качестве входного файла `initial_water_world.json`)*
```bash
python srge.py navigate 'Атом молекулы воды' --model qwen3 --input initial_water_world.json
```
...
```bash
python srge.py create "Наша Солнечная Система" --model qwen3:4b
```
...
#### **1. Использованный Промпт/Команда**
```bash
python srge.py create "Колония муравьев" --model qwen3:4b
```
...
#### **1. Использованный Промпт/Команда**
```bash
python srge.py create "Волшебный мир с рыцарями, магией и злодеями" --model qwen3:4b
```
...
#### **1. Использованный Промпт/Команда**
```bash
python srge.py create "Мир Cyberpunk 2077" --model qwen3:4b
```
...
#### **1. Использованный Промпт/Команда**
```bash
python srge.py create "Мир The Elder Scrolls V: Skyrim" --model qwen3:4b
```
...
#### **1. Использованный Промпт/Команда**
```bash
python srge.py create "Город, построенный внутри голов спящих титанов" --model qwen3:4b
```
...
#### **1. Использованный Промпт/Команда**
```bash
python srge.py create "Мир, состоящий только из чисел и их взаимосвязей" --model qwen3:4b
```
